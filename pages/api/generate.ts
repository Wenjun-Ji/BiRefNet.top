...
export default async function handler(
  ...
  let segmentedImage: string | null = null;
  const startTime = Date.now();
  const maxDuration = 300000; // 最大时间 300 秒

  while (!segmentedImage) {
    ...
    if (jsonFinalResponse.status === "succeeded") {
      segmentedImage = jsonFinalResponse.output;
  ...
  res
    .status(200)
    .json(segmentedImage ? segmentedImage : "Failed to segment image");
}
